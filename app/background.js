...
const compressImage = async (outputDir, filepath, jpegQuality) => {
  ...
  try {
    stats = await fs__WEBPACK_IMPORTED_MODULE_0___default().promises.stat(filepath);
    lstat = await fs__WEBPACK_IMPORTED_MODULE_0___default().promises.lstat(filepath);
  } catch (e) {
    return null;
  }
  ...
  let buffer = await fs__WEBPACK_IMPORTED_MODULE_0___default().promises.readFile(filepath);
  let width = buffer_image_size__WEBPACK_IMPORTED_MODULE_2___default()(buffer).width;
  let res = await sharp__WEBPACK_IMPORTED_MODULE_1___default()(buffer).resize({
...
/* harmony import */ var handbrake_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! handbrake-js */ "handbrake-js");
/* harmony import */ var handbrake_js__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(handbrake_js__WEBPACK_IMPORTED_MODULE_2__);




const compressVideo = async (outputdir, filepath, settings) => {
  let name = path__WEBPACK_IMPORTED_MODULE_1___default().basename(filepath);
  let outputname = outputdir + "/" + name;
  const options = {
    input: filepath,
  ...
  return await new (_babel_runtime_corejs3_core_js_stable_promise__WEBPACK_IMPORTED_MODULE_0___default())((resolve, reject) => {
    return handbrake_js__WEBPACK_IMPORTED_MODULE_2___default().spawn(options).on("error", err => {
      console.log(err);
...
/* harmony import */ var electron_store__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! electron-store */ "electron-store");
...
/* harmony import */ var _renderer_contants__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../../../renderer/contants */ "./renderer/contants.js");



const store = new (electron_store__WEBPACK_IMPORTED_MODULE_2___default())();

const getSetting = obj => store.get(obj.NAME) || obj.DEFAULT;

const getSettings = () => {
  // output directory
  const outputDir = getSetting(_renderer_contants__WEBPACK_IMPORTED_MODULE_3__.SETTINGS.OUTPUT.DIR); // if the media type will be compressed

  const isEnabled = {
    image: getSetting(_renderer_contants__WEBPACK_IMPORTED_MODULE_3__.SETTINGS.IMAGE.ENABLED),
    raw: getSetting(_renderer_contants__WEBPACK_IMPORTED_MODULE_3__.SETTINGS.RAW.ENABLED),
    video: getSetting(_renderer_contants__WEBPACK_IMPORTED_MODULE_3__.SETTINGS.VIDEO.ENABLED)
  }; // decide the quality
  ...
  const quality = {
    image: () => {
      ...
      return _babel_runtime_corejs3_core_js_stable_instance_map__WEBPACK_IMPORTED_MODULE_0___default()(_context = _babel_runtime_corejs3_core_js_stable_object_entries__WEBPACK_IMPORTED_MODULE_1___default()(_renderer_contants__WEBPACK_IMPORTED_MODULE_3__.SETTINGS.IMAGE.QUALITY)).call(_context, ([key, val]) => {
        return {
          key: getSetting(val)
        };
      });
    },
    raw: getSetting(_renderer_contants__WEBPACK_IMPORTED_MODULE_3__.SETTINGS.RAW.QUALITY),
    video: getSetting(_renderer_contants__WEBPACK_IMPORTED_MODULE_3__.SETTINGS.VIDEO.BITRATE)
  }; // format type (extension)

  const convert = {
    raw: getSetting(_renderer_contants__WEBPACK_IMPORTED_MODULE_3__.SETTINGS.RAW.CONVERT_TO),
    video: getSetting(_renderer_contants__WEBPACK_IMPORTED_MODULE_3__.SETTINGS.VIDEO.CONVERT.ENABLED) && getSetting(_renderer_contants__WEBPACK_IMPORTED_MODULE_3__.SETTINGS.VIDEO.CONVERT.TO)
  };
  return {
    outputDir,
    isEnabled,
